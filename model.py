...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # create a tensor of zeros as placeholder for even odd positions
        pe = torch.zeros(self.seq_len, self.d_model)
        # Create a vector of shape (seq_len)
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(
            0, d_model, 2, dtype=torch.float) * (-math.log(10000.0) / self.d_model))
...
class LayerNormalization(nn.Module):
    # eps: Needed to be considered for numerical stability.
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask):
        ...
        x = self.residual_connections[1](
            x, lambda x: self.cross_attention_block(x, x, encoder_output, src_mask))
